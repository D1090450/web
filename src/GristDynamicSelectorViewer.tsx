...
const GristDynamicSelectorViewer: React.FC = () => {
    ...
    const {
        ...
        sorting,
        setSorting,
        // 【主要變更點 1】: 接收新的 runQueryWithFilters 函數
        runQueryWithFilters,
    } = useGristData({
        apiKey,
        selectedDocId,
    ...
    return (
        <div style={styles.container as React.CSSProperties}>
            ...
            <div style={styles.card as React.CSSProperties}>
                ...
                {/* 【主要變更點 2】: 將 runQueryWithFilters 傳遞給 Filter 組件 */}
                {/* 只有在選定表格後才顯示篩選器 */}
                {selectedTableId && <Filter onSubmit={runQueryWithFilters} isLoading={isLoading} />}
            </div>
            )}
            
            {/* 只有在有欄位定義時才渲染表格，避免在切換表格時閃爍 */}
            ...
            {/* 當有查詢結果但為空時的提示 */}
            {totalRecords === 0 && selectedTableId && !isLoading && !dataError && (
              <p style={{textAlign: 'center', ...(styles.card as React.CSSProperties), marginTop: '20px'}}>找不到符合條件的紀錄。</p>
            )}
        </div>
    );
}
