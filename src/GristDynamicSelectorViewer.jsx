...
const styles = {
  appWrapper: {
    display: 'flex',
    alignItems: 'flex-start', // 改為 flex-start 以便從頂部開始，滾動條更自然
    justifyContent: 'center',
    minHeight: '100vh',
    backgroundColor: '#f0f2f5',
    padding: '60px 20px', // 增加頂部 padding
...
// --- 【修改】核心功能組件 (原 GristDynamicSelectorViewer) ---
function GristDynamicSelectorViewerCore() {
    ...
    useEffect(() => {
      ...
      const getOrgAndDocs = async () => {
        ...
        try {
          ...
          let determinedOrg = (Array.isArray(orgsData) && orgsData.length > 0)
            ? (orgsData.find(org => org.domain === TARGET_ORG_DOMAIN) || orgsData[0])
            : (orgsData && orgsData.id ? orgsData : null);
    ...
    const handleFetchTableData = useCallback(async () => {
      ...
      } catch (e) { setDataError('過濾條件不是有效的 JSON 格式。'); return; }
    ...
    const openGristLoginPopup = useCallback(() => {
      ...
      const pollForApiKey = async () => {
        ...
        if (success) { clearTimeout(pollingTimerRef.current); popup.close(); }
...
function GristDynamicSelectorViewer() {
    return (
        <div style={styles.appWrapper}>
            <GristDynamicSelectorViewerCore />
        </div>
    );
}
